...
const SEO_FILTERS = [
  { id: 'all', label: 'All SEO & Webmaster' },
  { id: 'seo-webmaster-tools', label: '🔍 Webmaster & Meta' },
  { id: 'social-media', label: '📱 Social Cards & Thumbnails' },
  { id: 'analytics', label: '📈 Analytics & Tracking' },
];

export function SeoWebmasterExplorer() {
  ...
  useEffect(() => {
    document.title = 'SEO Utilities & Social Media Optimization — iNWebTools';
    setLoading(true);
  ...
  const filteredTools = useMemo(() => {
    return tools.filter((t) => {
      ...
      if (activeFilter === 'social-media') {
        matchesFilter =
          t.slug.includes('graph') ||
          t.slug.includes('twitter') ||
          t.slug.includes('social') ||
          t.slug.includes('youtube');
      } else if (activeFilter === 'analytics') {
        matchesFilter =
          t.slug.includes('utm') || t.slug.includes('density') || t.slug.includes('serp');
      } else if (activeFilter !== 'all') {
        matchesFilter = t.subcategorySlug === activeFilter;
      }
    ...
    <div className="mx-auto max-w-7xl space-y-8 px-4 py-6 sm:px-6">
          ...
          <div className="max-w-2xl">
            <div className="flex items-center gap-3">
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-brand-500 to-accent-500 text-white shadow-lg shadow-brand-500/20 ring-1 ring-white/20">
                <CategoryIcon name="globe" className="h-6 w-6" />
              </span>
              <span className="rounded-full bg-brand-500/10 px-3 py-1 font-mono text-xs font-semibold text-brand-300 ring-1 ring-inset ring-brand-500/30">
                Phase 6 Engine • {tools.length} Tools Ready
              </span>
            </div>

            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
              SEO Utilities & Social Media Optimization
            </h1>
            <p className="mt-2 text-sm leading-relaxed text-slate-300">
              Complete webmaster suite for XML sitemaps, robots.txt, Google SERP simulator,
              Schema.org JSON-LD structured data, Open Graph & Twitter Cards, YouTube thumbnail
              extractors, and Google Analytics UTM campaign builders.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center">
              <div className="text-2xl font-black text-brand-400">Schema.org</div>
              <div className="text-[11px] text-slate-400 font-medium">JSON-LD Compliant</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center">
              <div className="text-2xl font-black text-emerald-400">100%</div>
              <div className="text-[11px] text-slate-400 font-medium">Google SERP Accurate</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center col-span-2 sm:col-span-1 lg:col-span-2 xl:col-span-1">
              <div className="text-2xl font-black text-accent-400">HD / 4K</div>
              <div className="text-[11px] text-slate-400 font-medium">YouTube Thumbnails</div>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-72">
          <input
            ...
            placeholder="Search SEO & Webmaster tools..."
      ...
      {loading ? (
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500/10 text-brand-400 group-hover:bg-brand-500 group-hover:text-white transition-colors">
                    <CategoryIcon name={t.icon || 'globe'} className="h-5 w-5" />
                  </span>
                  {t.isFeatured && (
        ...
        <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center">
          <p className="text-sm font-medium text-slate-300">No SEO tools found</p>

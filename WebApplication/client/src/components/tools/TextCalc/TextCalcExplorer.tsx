...
const TEXT_CALC_FILTERS = [
  { id: 'all', label: 'All Text & Calculators' },
  { id: 'text-metrics-analysis', label: '📊 Text Metrics & Readability' },
  { id: 'text-case-format-modifiers', label: '🔤 Case Converters' },
  { id: 'text-cleaning-manipulation', label: '🧹 Text Cleaning & Diff' },
  { id: 'financial-mathematical-calculators', label: '💰 Financial & Math Calculators' },
  { id: 'unit-converters', label: '⚖️ Unit Converters' },
];

export function TextCalcExplorer() {
  ...
  useEffect(() => {
    document.title = 'Text Utilities, Financial Calculators & Unit Converters — iNWebTools';
    setLoading(true);
  ...
  const crumbs: Crumb[] = [
    ...
    { label: 'Text Utilities & Calculators' },
  ];
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3">
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-brand-500 to-accent-500 text-white shadow-lg shadow-brand-500/20 ring-1 ring-white/20">
                <CategoryIcon name="type" className="h-6 w-6" />
              </span>
              <span className="rounded-full bg-brand-500/10 px-3 py-1 font-mono text-xs font-semibold text-brand-300 ring-1 ring-inset ring-brand-500/30">
                Phase 5 Engine • {tools.length} Tools Ready
              </span>
            </div>

            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
              Text Utilities, Financial Calculators & Unit Converters
            </h1>
            <p className="mt-2 text-sm leading-relaxed text-slate-300">
              High-speed in-memory engine for word and character counting, Flesch-Kincaid
              readability scoring, casing conversions, visual text diff comparison, duplicate line
              stripping, loan EMI & compound interest financial models, and multi-scale unit
              converters.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center">
              ...
              <div className="text-[11px] text-slate-400 font-medium">Casing Styles</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center col-span-2 sm:col-span-1 lg:col-span-2 xl:col-span-1">
              <div className="text-2xl font-black text-accent-400">0ms</div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:w-72">
          <input
            ...
            placeholder="Search text & calculators..."
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredTools.map((t) => (
            <Link
              key={t.slug}
              to={`/tools/text-calculators/${t.slug}`}
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500/10 text-brand-400 group-hover:bg-brand-500 group-hover:text-white transition-colors">
                    <CategoryIcon name={t.icon || 'type'} className="h-5 w-5" />
                  </span>
                  {t.isFeatured && (

...
const SECURITY_FILTERS = [
  { id: 'all', label: 'All Security Tools' },
  { id: 'security-cryptography', label: '🔐 Cryptography & Ciphers' },
  { id: 'network-web-diagnostics', label: '🌐 Network & DNS Diagnostics' },
  { id: 'seo-webmaster-tools', label: '🛡️ SSL & Security Headers' },
];

export function SecurityNetworkExplorer() {
  ...
  useEffect(() => {
    document.title = 'Security, Cryptography & Network Diagnostics — iNWebTools';
    setLoading(true);
  ...
  const crumbs: Crumb[] = [
    ...
    { label: 'Security & Network Diagnostics' },
  ];
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="max-w-2xl">
            <div className="flex items-center gap-3">
              <span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-tr from-brand-500 to-accent-500 text-white shadow-lg shadow-brand-500/20 ring-1 ring-white/20">
                <CategoryIcon name="shield" className="h-6 w-6" />
              </span>
              <span className="rounded-full bg-brand-500/10 px-3 py-1 font-mono text-xs font-semibold text-brand-300 ring-1 ring-inset ring-brand-500/30">
                Phase 4 Engine • {tools.length} Tools Ready
              </span>
            </div>

            <h1 className="mt-4 text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
              Security, Cryptography & Network Diagnostics
            </h1>
            <p className="mt-2 text-sm leading-relaxed text-slate-300">
              Enterprise-grade toolkit for cryptographic hashing (SHA-256, SHA-512, MD5, Keccak,
              CRC32), AES-256 symmetric ciphers, high-entropy password generators, RSA & Ed25519 key
              pair generation, IPv4 subnet calculators, DNS records query, and SSL certificate
              inspectors.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3">
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center">
              <div className="text-2xl font-black text-brand-400">4096-bit</div>
              <div className="text-[11px] text-slate-400 font-medium">RSA & Ed25519 Keys</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center">
              <div className="text-2xl font-black text-emerald-400">AES-256</div>
              <div className="text-[11px] text-slate-400 font-medium">Military Encryption</div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 text-center col-span-2 sm:col-span-1 lg:col-span-2 xl:col-span-1">
              ...
              <div className="text-[11px] text-slate-400 font-medium">Zero-Knowledge Sandbox</div>
            </div>
          </div>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredTools.map((t) => (
            <Link
              key={t.slug}
              to={`/tools/security-network/${t.slug}`}
              ...
              <div>
                <div className="flex items-center justify-between">
                  <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-brand-500/10 text-brand-400 group-hover:bg-brand-500 group-hover:text-white transition-colors">
                    <CategoryIcon name={t.icon || 'shield'} className="h-5 w-5" />
                  </span>
                  {t.isFeatured && (
        ...
        <div className="rounded-2xl border border-dashed border-white/10 p-12 text-center">
          <p className="text-sm font-medium text-slate-300">No security tools found</p>
